...
func TestCodexParse(t *testing.T) {
	...
	parser := ai.Codex{
		...
		UserAgents: map[string]string{
			"/home/user/projects/hackatime-cli/pkg/ai/claude.go": heartbeat.UserAgent(ctx, "editor/1.2.3"),
		},
	}
	...
	assert.Equal(t, "/root/hackatime-cli", got[0].ProjectPathOverride)
	...
	assert.Equal(t, "/root/hackatime-cli", got[1].ProjectPathOverride)
	...
	assert.Equal(t, "/home/user/projects/hackatime-cli/pkg/ai/claude.go", got[2].Entity)
	...
	transcript := strings.Join([]string{
		...
		strings.Join([]string{
			...
			`"payload":{"type":"user_message","message":"# Context from my IDE setup:\n\n` +
				`## Active file: hackatime-cli/static/css/index.less\n\n## Open tabs:\n` +
				`- index.less: hackatime-cli/static/css/index.less\n\n## My request for Codex:\n` +
...
func TestCodexParse_RolloutFixtureIncludesExpectedHeartbeatAttributes(t *testing.T) {
	...
	assert.Equal(t, "/Users/user/git/hackatime-cli", heartbeats[i].ProjectPathOverride)
	...
	assert.Equal(t, "/Users/user/git/hackatime-cli/templates/index.html", heartbeats[i].Entity)
	assert.Equal(t, "ai coding", heartbeats[i].Category)
	...
	assert.Equal(t, heartbeat.FileType, heartbeats[i].EntityType)
	assert.Equal(t, "/Users/user/git/hackatime-cli/static/css/index.less", heartbeats[i].Entity)
	assert.Equal(t, "ai coding", heartbeats[i].Category)
	assert.True(t, *heartbeats[i].IsWrite)
	assert.Zero(t, heartbeats[i].AIInputTokens)
	assert.Zero(t, heartbeats[i].AIOutputTokens)
