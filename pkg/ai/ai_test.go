...
func TestSendHeartbeats_WithAIParsing(t *testing.T) {
	...
	router.HandleFunc("/users/current/heartbeats.bulk", func(w http.ResponseWriter, req *http.Request) {
		...
		assert.Equal(t, "hackatime-cli", *entities[0].Project)
...
func TestWithAISync_ProducesExpectedHeartbeats(t *testing.T) {
	...
	assert.Equal(t, "/Users/user/git/hackatime-cli/templates/index.html", h.Entity)
	...
	assert.Equal(t, "/Users/user/git/hackatime-cli", h.ProjectPathOverride)
	...
	assert.Equal(t, heartbeat.FileType, h.EntityType)
	assert.Equal(t, "/Users/user/git/hackatime-cli/static/css/index.less", h.Entity)
	assert.Equal(t, "ai coding", h.Category)
	assert.True(t, *h.IsWrite)
